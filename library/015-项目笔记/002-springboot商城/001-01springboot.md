...
new project -------------------->empty Projet

工程名为demo：

新建一个model：new --------> Module

使用maven来构建：maven---------->配置module SDK(create from a archetype)---------> next
...
包括：日志：logback 、log4j、slf4j

​	   	json : jackson

​		spring :aop、bean、context、core、expression、web、webMVC

​		springboot ：核心包、autoconfigure、starter、 json、tomcat、web启动包
...
> 1. 配置组件扫描的指令。提供了类似与`<context:component-scan>`标签的作用
> 
...
包括：admin、aop、cache、cloud、context、dao、date、jdbc等
